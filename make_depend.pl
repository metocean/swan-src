...
use strict;
use Getopt::Std;
use vars qw( $opt_h $opt_v $opt_d $opt_s $opt_f $opt_o $opt_p $opt_l $opt_n );
...
sub usage {
  ...
  print STDERR " -n            : do not recursively process directories\n";
  print STDERR " [dirs]        : directories for the recursive search; default is .\n";
  exit $c;
}

# process optional arguments
getopts( 'hvdsf:o:p:ln' ) || &usage;
&usage(0) if $opt_h;
...
traverse_dir_trees( \&process_files, \@tdirs, $skip_dirs, $opt_d, $opt_n, $opt_v );
...
sub process_files {
  ...
  my $suffix = qr/\.[^.]*/;
  my $curdir = qr/\$\(CURDIR\)/;
  my $t; my $f; my $m; my @files=(); my @mods=(); my $odir;
...
#-process Fortran sources
  ...
  foreach $f ( @files ) {
    ...
    ($t = $f) =~ s/$suffix/.o/;
    if ( $opt_p ) { if ( $opt_p !~ m/NONE/ ) { $t = $opt_p."/".$t; } }
    elsif ( $odir ) { $t = $odir."/".$t; }
    else { $t = $d."/".$t; }
    push( @{$tgts{$d}}, $t );
...
sub traverse_dir_trees {
  my ($func, $tdirs, $sdirs, $opt_d, $opt_n, $opt_v) = @_;
  my $sub = (caller(0))[3];
